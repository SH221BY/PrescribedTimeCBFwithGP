...
%% h1_learn
Len = length(Q_learn);
Q_const = [Q_learn(1:2,:);ones(1,Len)];
h1 = LocalPreCBFGP.CBFProperty.H_offset*Q_const;
...
%% h1_un
Len = length(Q_uncertainty);
Q_const = [Q_uncertainty(1:2,:);ones(1,Len)];
h1 = LocalPreCBFGP.CBFProperty.H_offset*Q_const;
...
for i = 1:Len
    un_r(:,i) = GenerateUncertainty(Q_learn(1:2,i),Q_learn(3:4,i));
    [mu_r(:,i),var,eta_r(:,i),beta,gamma,eta_min] = LocalGP.predict(Q_learn(1:4,i));
